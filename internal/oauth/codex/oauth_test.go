...
func TestParseRedirectURL(t *testing.T) {
	...
	code, state, err := ParseRedirectURL("http://localhost:1455/auth/callback?code=authorization-code&state=csrf-state")
	require.NoError(t, err)
	require.Equal(t, "authorization-code", code)
	require.Equal(t, "csrf-state", state)
}

func TestParseRedirectURLRequiresAuthorizationCode(t *testing.T) {
	t.Parallel()

	code, state, err := ParseRedirectURL("http://localhost:1455/auth/callback?state=csrf-state")
	require.Error(t, err)
	require.Empty(t, code)
	require.Empty(t, state)
}
