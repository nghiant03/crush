...
func TestCodexResponsesRoundTripPassesThroughOtherRequests(t *testing.T) {
	t.Parallel()

	const requestBody = `{"input":[{"role":"user","content":"Hello"}]}`
	calls := 0
	rt := codexInstructionsRoundTripper{
		next: roundTripFunc(func(req *http.Request) (*http.Response, error) {
			calls++
	...
	req, err := http.NewRequest(http.MethodPost, "https://api.openai.com/v1/chat/completions", strings.NewReader(requestBody))
	require.NoError(t, err)
...
func TestCodexResponsesRoundTripStopsWhenCanceled(t *testing.T) {
	t.Parallel()
	...
	req, err := http.NewRequestWithContext(ctx, http.MethodPost, "https://chatgpt.com/backend-api/codex/responses", strings.NewReader(`{}`))
	require.NoError(t, err)
